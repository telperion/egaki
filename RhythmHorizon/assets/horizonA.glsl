...
uniform float aurora_speed = 0.1;
uniform float aurora_submerge = 0.7;
...
void main()
{
    ...
    vec4 texV = texture2D(horizon_src_tex, vec2(mod(aurora_speed * t, 1.0), uv.x))*aurora_submerge;
    vec4 texG = texture2D(texture, pq);

    gamma = 1.0 - lightener * grey(texG.rgb);
    
    vec4 pal = PalettePick((0.5 * uv.y / grey(texH.rgb) + grey(texV.rgb)) / max_height, hsv_blend);
    pal.rgb *= 1.0 - aurora_dim*grey(texV.rgb);
    ...
    // Output to screen
    gl_FragColor = vec4(pow(color_sum.rgb, vec3(gamma)), color_sum.a);
}